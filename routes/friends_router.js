...
router.get("/", function (request, response) {
  ...
  db.query(
    `SELECT u.name, f.friend2 FROM userTBL AS u JOIN friendTBL AS f ON f.friend2 = u.email 
    ...
    function (error, friend) {
      ...
      for (let i = 0; i < friend.length; i++) {
        friendsList += friendsTemplate.friendsList(
          friend[i].friend2,
          friend[i].name
        );
      }
      ...
      db.query(
        ...
        function (error, group) {
          ...
          for (let i = 0; i < group.length; i++)
            groupList += friendsTemplate.groupList(
              group[i].g_id,
              group[i].g_name
            );
...
router.post("/findFriends", function (request, response) {
  ...
  db.query(
    ...
    function (error, user) {
      db.query(
        `SELECT * FROM userTBL WHERE email='${f_email}'`,
        function (error, f) {
...
router.post("/reqFriends", function (request, response) {
  ...
  db.query(
    `INSERT INTO addFriendTBL VALUES('${reqSender}', '${reqReceiver.friend}')`,
    function (error) {
      if (error) {
        console.log(error);
        let alert = `
...
router.post("/addGroup_process", function (request, response) {
  ...
  db.query(
    `INSERT INTO groupTBL(g_owner, g_name) VALUES('${g_owner}', '${g_name}')`,
    function (error1) {
      if (error1) {
        console.log(error1);
        throw error1;
      ...
      db.query(`SELECT LAST_INSERT_ID()`, function (error2, result) {
        ...
        for (let i = 0; i < g_member.length; i++) {
          db.query(
            `INSERT INTO memberTBL(m_id, m_email) VALUES(?, ?)`,
            [result[0]["LAST_INSERT_ID()"], g_member[i]],
            function (error3) {
              if (error3) {
                console.log(error3);
                throw error3;
...
router.post("/delSelected", function (request, response) {
  ...
  if (Array.isArray(g_id)) {
    for (let i = 0; i < g_id.length; i++) {
      db.query(`DELETE FROM memberTBL WHERE m_id=${g_id[i]}`, function (error) {
        ...
        db.query(
          `DELETE FROM groupTBL WHERE g_id = ${g_id[i]}`,
          function (error) {
            if (error) {
              console.log(error);
              throw error;
  ...
  if (Array.isArray(f_email)) {
    for (let i = 0; i < f_email.length; i++) {
      db.query(
        `DELETE FROM memberTBL WHERE m_email='${f_email[i]}'`,
        function (error) {
          if (error) {
            console.log(error);
            throw error;
          ...
          db.query(
            `DELETE FROM friendTBL WHERE (friend1 = '${email}' AND friend2 = '${f_email[i]}') OR
                (friend1 = '${f_email[i]}' AND friend2 = '${email}')`,
    ...
    db.query(
      `DELETE FROM memberTBL WHERE m_email='${f_email}'`,
      function (error) {
        if (error) {
          console.log(error);
          throw error;
        ...
        db.query(
          `DELETE FROM friendTBL WHERE (friend1 = '${email}' AND friend2 = '${f_email}') OR
            (friend1 = '${f_email}' AND friend2 = '${email}')`,
          function (error, result) {
            if (error) {
              console.log(error);
              throw error;
...
router.get("/sentReq", function (request, response) {
  ...
  db.query(
    `SELECT af.user1, af.user2, u.name from addFriendTBL AS af JOIN userTBL As u ON u.email=af.user2
    WHERE af.user1='${email}'`,
    function (error, sentReq) {
      for (let i = 0; i < sentReq.length; i++) {
        sentReqList += friendsTemplate.friendsList(
          sentReq[i].user2,
          sentReq[i].name
        );
      }
...
router.get("/receivedReq", function (request, response) {
  ...
  db.query(
    `SELECT af.user1, af.user2, u.name from addFriendTBL AS af JOIN userTBL As u ON u.email=af.user1
    WHERE af.user2='${email}'`,
    function (error, receivedReq) {
      for (let i = 0; i < receivedReq.length; i++) {
        receivedReqList += friendsTemplate.friendsList(
          receivedReq[i].user1,
          receivedReq[i].name
        );
      }
...
router.post("/cancelReq", function (request, response) {
  ...
  if (Array.isArray(f_email)) {
    for (let i = 0; i < f_email.length; i++) {
      db.query(
        `DELETE FROM addFriendTBL WHERE user1='${email}' AND user2='${f_email[i]}'`,
        function (error) {
          if (error) {
            console.log(error);
            throw error;
    ...
    db.query(
      `DELETE FROM addFriendTBL WHERE user1='${email}' AND user2='${f_email}'`,
      function (error) {
        if (error) {
          console.log(error);
          throw error;
...
router.post("/confirmReq", function (request, response) {
  ...
  if (Array.isArray(f_email)) {
    for (let i = 0; i < f_email.length; i++) {
      db.query(
        `INSERT INTO friendTBL(friend1, friend2) VALUES('${f_email[i]}', '${email}')`,
        function (error) {
          if (error) {
            console.log(error);
            throw error;
          ...
          db.query(
            `INSERT INTO friendTBL(friend1, friend2) VALUES('${email}', '${f_email[i]}')`,
            function (error) {
              if (error) {
                console.log(error);
                throw error;
              ...
              db.query(
                `DELETE FROM addFriendTBL WHERE user1='${f_email[i]}' AND user2='${email}'`,
                function (error) {
                  if (error) {
                    console.log(error);
                    throw error;
    ...
    db.query(
      `INSERT INTO friendTBL(friend1, friend2) VALUES('${f_email}', '${email}')`,
      function (error) {
        if (error) {
          console.log(error);
          throw error;
        ...
        db.query(
          `INSERT INTO friendTBL(friend1, friend2) VALUES('${email}', '${f_email}')`,
          function (error) {
            if (error) {
              console.log(error);
              throw error;
            ...
            db.query(
              `DELETE FROM addFriendTBL WHERE user1='${f_email}' AND user2='${email}'`,
              function (error) {
                if (error) {
                  console.log(error);
                  throw error;
